...
#ifdef CHISLINK
EWRAM_DATA u32 save_start = 0x6000;
EWRAM_BSS volatile u32 chislink_save_dirty_mask;
EWRAM_BSS volatile u16 chislink_save_idle_frames;
EWRAM_BSS static u32 chislink_save_bytes;
EWRAM_BSS static u8 chislink_save_ready;
#endif
int get_saved_sram(void)
{
	#ifdef CHISLINK
	chislink_save_dirty_mask = 0;
	chislink_save_idle_frames = 0;
	chislink_save_bytes = 0;
	chislink_save_ready = 0;
	if ((g_cartflags & 2u) == 0 || g_rammask == 0) return 0;

	u32 bytes = g_rammask + 1u;
	if (bytes > sizeof(XGB_SRAM)) return 0;
	chislink_io_pause_state_t pause = chislink_io_pause();
	int ret = chislink_bridge_save_open();
	u32 loaded = 0;
	if (ret == 0)
	{
		ret = chislink_bridge_save_read(XGB_SRAM, bytes, &loaded);
	}
	chislink_io_resume(pause);
	if (ret < 0) return 0;
	chislink_save_bytes = bytes;
	chislink_save_ready = 1;
	return loaded != 0;
	#else
	return 0;
	#endif
...
#ifdef CHISLINK
void chislink_save_poll(void)
{
	u32 dirty = chislink_save_dirty_mask;
	if (!chislink_save_ready || dirty == 0) return;
	if (chislink_save_idle_frames < 30u)
	{
		++chislink_save_idle_frames;
		return;
	}

	u32 page = 0;
	while ((dirty & (1u << page)) == 0) ++page;
	u32 offset = page * 1024u;
	if (offset >= chislink_save_bytes)
	{
		chislink_save_dirty_mask &= ~(1u << page);
		return;
	}
	u32 length = chislink_save_bytes - offset;
	if (length > 1024u) length = 1024u;

	chislink_io_pause_state_t pause = chislink_io_pause();
	int ret = chislink_bridge_save_write(offset, XGB_SRAM + offset, length);
	chislink_io_resume(pause);
	if (ret == 0)
	{
		chislink_save_dirty_mask &= ~(1u << page);
