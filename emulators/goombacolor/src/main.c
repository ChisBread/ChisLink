...
#if MOVIEPLAYER
EWRAM_BSS int usinggbamp;
EWRAM_BSS int usingcache;
...
void C_entry()
{
	...
	#endif
	#ifdef CHISLINK
	if (chislink_bridge_open(CLP_LAUNCH_GAMEBOY) < 0)
	{
		while (1) {}
	}
	usingcache = 1;
	...
	*timeregs=1;
	...
	#ifdef CHISLINK
	chislink_bridge_install_graphics_irqs(chislink_vblank_irq_dispatch,
		chislink_vcount_irq_dispatch);
	#else
	GFX_init_irq();
	#endif
//	vcountfptr=&vbldummy;
...
u8 *findrom2(int n)
{
	...
	if (!launch) return 0;
	u8 *scratch = bank_1;
	if (cache_location[0] == bank_1 && prgcache[0] == 0) return bank_1;
	memset(scratch, 0xff, 16 * 1024);
	u32 first_page = launch->size < 16 * 1024 ? launch->size : 16 * 1024;
	chislink_io_pause_state_t pause = chislink_io_pause();
	int ret = chislink_bridge_read_words(0, scratch, first_page);
