...
void init_cache(u8* nes_header, int do_reset)
{
	...
	if (has_vram) memset32(NES_VRAM, 0, 8192);
	if (fourscreen) memset32(NES_VRAM4, 0, 2048);
	(void)allocate_end_variables(do_reset);
	(void)chislink_pocketnes_cache_init();
	paletteinit();
	resume_hdma();
}
