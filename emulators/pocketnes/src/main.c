...
APPEND int main()
{
	...
	#ifdef CHISLINK
	ewram_start = (u8*)end_addr;
	if (chislink_pocketnes_boot() < 0)
	{
		while (1) {}
	}
	extern u8 END_OF_EXRAM;
	if (chislink_pocketnes_cache_reserve(&ewram_start, &END_OF_EXRAM) < 0)
	{
		while (1) {}
	}
	heap_addr = (u32)ewram_start;
	textstart = chislink_pocketnes_rom();
	#endif
...
APPEND void release_vram(bool clear)
{
	...
	irqEnable(IRQ_KEYPAD | IRQ_VBLANK | IRQ_TIMER1 | IRQ_TIMER2 | IRQ_TIMER3 |
		IRQ_SERIAL);
...
#if GCC
	#ifdef CHISLINK
	extern u8 __chislink_stack_top[];
	u32 newstack=(u32)(&__chislink_stack_top);
	#else
	extern u8 __sp_usr[];
